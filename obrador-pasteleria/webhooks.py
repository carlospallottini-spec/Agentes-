...
app = FastAPI(title="Obrador de Pastelería - Asistente social")
...
def _already_processed(message_id: str | None) -> bool:
    """Dedupe simple en memoria — Meta reintenta webhooks si no respondés 200."""
    if not message_id:
        return False
    with _PROCESSED_LOCK:
        ...
        # Cap rudimentario para no crecer indefinidamente
...
def _handle_message(channel: str, sender: str, text: str, send_fn) -> None:
    """Lógica común: cargar historial, correr el agente, responder por el canal."""
...
@app.get("/")
def health() -> dict:
    return {"status": "ok", "bakery": "obrador-pasteleria"}
